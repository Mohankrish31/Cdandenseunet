...
class DenseUNetCBAM(nn.Module):
   ...
   # ------------------------ CDAN DenseUNet Wrapper (Updated) ------------------------
class CDANDenseUNet(nn.Module):
    def __init__(self, in_channels=3, base_channels=32, num_classes=2):
        super().__init__()
        ...
        self.discriminator = None  # will be initialized lazily
    def forward(self, x):
        out = self.generator(x)          # generator output (RGB)
        pooled = self.pool(out)          # global feature pooling
        feat_vec = pooled.view(pooled.size(0), -1)
        return out, feat_vec
    def init_discriminator(self, feat_dim):
        """Initialize discriminator for num_classes"""
        self.discriminator = CDANDiscriminator(feat_dim, self.num_classes, hidden=1024)
    def domain_classify(self, feat_vec, soft_preds, grl_lambda=None):
        if self.discriminator is None:
            self.init_discriminator(feat_vec.size(1))
